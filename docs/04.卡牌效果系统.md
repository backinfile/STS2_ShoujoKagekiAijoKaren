...
```csharp
// 静态声明（在 CanonicalKeywords 中，仅用于基础关键词）
...
> **重要**：`CanonicalKeywords` 仅在卡牌初始化时读取一次用于构建 `Keywords` 集合。升级后新增的关键词（如 `Innate`）**不应**在 `CanonicalKeywords` 中使用 `IsUpgraded` 条件判断，而应在 `OnUpgrade()` 中调用 `AddKeyword(CardKeyword.Innate)` 动态添加。
